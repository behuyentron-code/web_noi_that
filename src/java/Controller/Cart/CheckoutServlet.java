...
public class CheckoutServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try {
            ...
            String sqlOrder = "INSERT INTO orders (user_id, shipping_address, payment_method, shipping_fee, discount, subtotal, total_price, status, order_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
            psOrder = conn.prepareStatement(sqlOrder, PreparedStatement.RETURN_GENERATED_KEYS);
            psOrder.setInt(1, userId);
            psOrder.setString(2, address + " - " + fullname + " - " + phone);
            psOrder.setString(3, paymentMethod);
            psOrder.setDouble(4, shipping);
            psOrder.setDouble(5, 0); // discount
            psOrder.setDouble(6, subtotal);
            psOrder.setDouble(7, total);
            psOrder.setString(8, "pending");
            psOrder.setTimestamp(9, new Timestamp(System.currentTimeMillis()));
